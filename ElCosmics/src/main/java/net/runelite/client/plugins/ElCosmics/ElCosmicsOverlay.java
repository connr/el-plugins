...
@Slf4j
@Singleton
class ElCosmicsOverlay extends OverlayPanel
{
    private final ElCosmicsPlugin plugin;
    private final ElCosmicsConfig config;

    String timeFormat;

    @Inject
    private ElCosmicsOverlay(final Client client, final ElCosmicsPlugin plugin, final ElCosmicsConfig config)
    {
        super(plugin);
    ...
    {
        if (plugin.botTimer == null || !plugin.startCosmics || !config.enableUI())
        {
