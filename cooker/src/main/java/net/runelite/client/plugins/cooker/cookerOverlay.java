...
class cookerOverlay extends OverlayPanel
{
	...
	private cookerOverlay(final Client client, final cookerPlugin plugin, final cookerConfiguration config)
	{
		...
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "karambwan fisher overlay"));
	}

	@Override
	public Dimension render(Graphics2D graphics)
	{
		if (plugin.botTimer == null || !plugin.startCooker || !config.enableUI())
		{
